...
e.	Methods
### Sample Description
The study involved 1000 participants and eight attributes of these participants were observed, measured, and recorded in a dataset called mydata. A replication sample measurunng similar attributes was obtained with 300 participants. The attributes include that were measured include;
- **Sex** were particiants were categorized as 0 and 1,
- **Testosteron**: Measured from saliva in the morning,
- **fMRI Activation**: Measured in the amygdala and hippocampus regions of the brain through MRI images encoded in phases, comparing the average activity of the amygdala/hippocampus when encoding negative vs. neutral images,
- **Memory Performance (EM_SD and EM_LD)**: Assessed using tests of episodic memory for pictures, taken 10 min (Short Delays) or after 24 hours(Long Delay) after encoding,
- **Extraversion**: Determined based on a standard personality test.

### Statistical Analysis
The following statisitical steps were performed in the analysis:
1. `Data Filtering`: Removed entries with 'Filter' equal to 1 because the quality control was not done for participants with this entry 
2. `Validation Steps`: Checked the reliability of Testosterone and fMRI measurements through box plots, t tests, correlation tests and scatter plots.
3. `Aggregation`: Aggregated fMRI variables based on collinearity measured with a correlation test to form an Aggregated_fMRI column
4. `Main Analysis`: Conducted multivariate regression models and measured interaction effects for memory measurements, considering Aggregated_fMRI, and Sex as predictors.

## Results
### Filtering & Validation
The data cleaning process resulted in 833 entries remaining after filtering in the main data set and 247 in the replication sample.

Validation checks revealed significant differences in Testosterone levels between sexes. 
```r
t_test_1 <- t.test(Testosteron ~ Sex, data = mydata_1)
print(t_test_1)
p-value = 2.2e-16; #null hypothesis is rejected; #Group 1 have a significantly higher mean value (23.191896) compared to group 0 (1.219012)
```

A correlation test on two brain signals returned a high positive correlation coefficient
```r
corr_coefficient <- cor(mydata_1$fMRI_amy_neg_neu, mydata_1$fMRI_hipp_neg_neu)
print(corr_coefficient)
[1] 0.7487487
```

A correlation test between the memory measurements during short and long delays returned a high positive correlation and measurement of the performance difference between short delay and long delay conditions with a t test showed a significant difference in performance with short delay conditions returning significantly higher measurements compared to long delays
```r
correlation_coefficient <- cor(mydata_1$EM_SD, mydata_1$EM_LD)
print(correlation_coefficient)
[1] 0.8569001
```
```r
t_test_2 <- t.test(mydata_1$EM_SD, mydata_1$EM_LD, paired = TRUE)
print(t_test_2)
p-value < 2.2e-16; #null hypothesis rejected
```

### Main Research Question Results
The main research question explored the influence of fMRI activation on memory performance in conditions of short delays and long delays between the two sexes. 
Results of the multivariate regression models indicated a significant relationship between Aggregated_fMRI and the memory performance in both short and long delay measurements with sex1 (males) showing a statistically lower memory perfomance to sex 0 (females)
```r
model_SD1 <- lm(EM_SD ~ Aggregated_fMRI + Sex, data = mydata_1)
model_LD1 <- lm(EM_LD ~ Aggregated_fMRI + Sex, data = mydata_1)
summary(model_SD1)
# Coefficient for Aggregated_fMRI = 4.1483, and p value = 2e-16
# Coefficient for Sex1 = -0.9620 and p value = 0.0388
# R-squared = 0.5614
summary(model_LD1)
# Coefficient for Aggregated_fMRI = 3.7113 and p value = 2e-16
# Coefficient for Sex1 = -3.7699 and p value = 1.22e-10
# R-squared = 0.4297
```

On measuring the interaction effect of Aggregated_fMRI and Sex on memory performance, results for short delay showed a statistically non-significant relationship implying that the relationship between Aggregated_fMRI and EM_SD does not significantly differ between the two categories of Sex while long delay measurements showed a statistically significant relationship, implying that the relationship between Aggregated_fMRI on EM_LD differs for different Sexes.
```r
summary(SD_interaction)
# Coefficient for Aggregated_fMRI = 4.3161 and p value = 2e-16
# Coefficient for Sex1 = -0.5081 and p value = 0.653    
# Coefficient for Aggregated_fMRI:Sex1 = -0.3728 and p value = 0.154
summary(LD_interaction)
# Coefficient for Aggregated_fMRI = 4.0905 and p value = 2e-16
# Coefficient for Sex1 = -0.4464 and p value = 0.75008    
# Coefficient for Aggregated_fMRI:Sex1 = -0.8428 and p value = 0.00942
```

### Sensitivity Analysis
A sensitivity analysis without applying the initial filter(see above) showed that the direction and magnitude of changes in estimates and standard errors are relatively small, indicating that the coefficients are stable across the main and sensitivity analyses. However, the p value of the Sex variable showed a marked change throughout. This suggests sensitivity of the sex variable to the initial data filter].
...
still only good vibes

Wir haben ca. 2.5 Stunden gebraucht. Die Aufgaben waren teils etwas schwierig, da keiner von uns im Umgang mit GitHub vertraut war. Es ging dann aber.
